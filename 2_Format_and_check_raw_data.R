...
data <- read.csv('/Path/to/main_directory/raw_trial_data.csv', header = FALSE)
backup_data <- data
...
colnames(data) <- c("Trial_Overall", "Block", "Trial_WithinBlock", "ArraySetSize", "Change", "RT", "ID")
colnames(data_orig) <- c("Trial_Overall", "Trial_WithinBlock", "Block", "ArraySetSize", "Change", "RT", "ID")

#Reorder columns so they are the same

data <- data[, c("ID", "Trial_Overall", "Trial_WithinBlock", "Block", "ArraySetSize", "Change", "RT")]
data_orig <- data_orig[, c("ID", "Trial_Overall", "Trial_WithinBlock", "Block", "ArraySetSize", "Change", "RT")]

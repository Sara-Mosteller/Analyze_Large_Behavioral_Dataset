...
%After downloading the data, create a main directory with Data_Experiment_2
...
%This loop will skip over the 5 dummy files, which cannot be processed by the script. 
% Additionally, the file 382903_ColorK.mat cannot be loaded; once removed from the folder,
...
for k = 1 : length(allFiles)
    ...
    probeLoc = reshape(stim.probeLoc, 24*5, 2);
    ...
    for i = 1:length(matrixlist)
        out{i} = reshape(stim.(matrixlist{i}), 24*5, []);
    end
    ...
    %Create the remaining variables
    ID = p.subNum * ones(120, 1);
    block =  transpose(repelem(1:5, 24)); 
    trial_num_overall = transpose(1:120);
    trial_num_within_block = transpose(repmat(1:24, 1, 5)); 
    stim_duration = prefs.stimulusDuration * ones(120, 1);
    ITI = prefs.ITI * ones(120, 1);
    retention_interval = prefs.retentionInterval * ones(120, 1);
    break_duration = prefs.breakLength * ones(120, 1);
    stim_size = prefs.stimSize * ones(120, 1);
    fixation_size = prefs.fixationSize * ones(120, 1);
    min_distance = prefs.minDist * ones(120, 1);
    ...
    writematrix(partdata, 'Expt_2_raw_trial_data.csv', 'WriteMode', 'append')

    clearvars -except allFiles

end

  %% Add column names to the CSV file, finish formatting and resave

 full_dataset = readtable('Expt_2_raw_trial_data.csv');

 full_dataset.Properties.VariableNames = {'id_placeholder', 'trial_num', 'block', 'trial_num_within_block',...
     ...
     'item_color_5', 'item_color_6', 'item_color_7', 'item_color_8', 'item_location_1_x',...
     'item_location_2_x', 'item_location_3_x', 'item_location_4_x', 'item_location_5_x',...
     'item_location_6_x', 'item_location_7_x', 'item_location_8_x', 'item_location_1_y',...
 ...
 %Carry out final formatting by separating the ID, session, and place information.

n = height(full_dataset);
...
for i = 1:n
    s = num2str(full_dataset.id_placeholder(i));   % convert number to string
    L(i) = length(s);
    ...
    elseif L(i) == 6
        ...
        fprintf('Unexpected ID length: %s %d\n', s, i);
         id_i = '999';
         session_i = '99';
         place_i = '9';
    end
    
    id_i = str2double(id_i);
    session_i = str2double(session_i);
    place_i = str2double(place_i);
...
% Add to table
full_dataset.id = id;
full_dataset.session = session;
full_dataset.place = place;

%Reorder the columns such that id, session and place are at the beginning.
full_dataset = movevars(full_dataset, {'id', 'session', 'place'}, 'Before', 'trial_num');

%Option to delete the id_placeholder columm before writing the CSV file:
%full_dataset = removevars(full_dataset, 'id_placeholder');

writetable(full_dataset, 'Experiment_2_trial_data.csv');

%It is necessary to manually fix the ids without lengths of 5 or 6 in the
%placeholder column. These cases can be identified by searching for place = 9. 

...
#For a trimmed version of the dataset, eliminate all but the first 160 trials, so that there are 120 trials of set size 4 and 8
dataset1_expt1_trimmed <-  dataset1_expt1 %>% group_by(id) %>% slice_head(n = 160) %>% ungroup()

#Select the overlapping columns
dataset1_expt1_trimmed <- subset(dataset1_expt1_trimmed, select = c('study', 'experiment', 'id', 'session', 'trial_num', 'block', 'trial_num_within_block', 'set_size', 'change', 'response', 'rt', 'fixation_size', 'stim_size', 'stim_duration', 'retention_interval', 'distance_from_monitor', 'test_location_x_from_center', 'test_location_y_from_center'))

#Check the number of unique IDs
length(unique(dataset1_expt1$id)) #should be 135

dim(dataset1_expt1_trimmed) #Should be 21600 rows and 17 columns
...
#Eliminate all but the first session (120 trials on the first day and 48 trials on the second day)
dataset1_expt2_trimmed <- dataset1_expt2 %>% group_by(id) %>% slice_head(n = 168) %>% ungroup()

#Select the overlapping columns
dataset1_expt2_trimmed <- subset(dataset1_expt2_trimmed, select = c('study', 'experiment', 'id', 'session', 'trial_num', 'block', 'trial_num_within_block', 'set_size', 'change', 'response', 'rt', 'fixation_size', 'stim_size', 'stim_duration', 'retention_interval', 'distance_from_monitor', 'test_location_x_from_center', 'test_location_y_from_center'))

dim(dataset1_expt2_trimmed) #Should be 13272 rows out of the original 293880
#Check the number of unique IDs
...
###########################################################################################################
#Create and write the combined datasets
###########################################################################################################

#Create a version with the trimmed datasets
data <- rbind(dataset1_expt1_trimmed, dataset1_expt2_trimmed, dataset2)
...
data_final <- subset(data, select = c('study', 'experiment', 'id', 'session', 'trial_num','set_size', 'change', 'response', 'rt', 'stim_size', 'fixation_size', 'stim_duration', 'retention_interval', 'distance_from_monitor', 'test_location_x_from_center', 'test_location_y_from_center'))
head(data_final)
#Check the dimensions
dim(data_final) #495768 rows, 16 columns
#Check the number of unique IDs
length(unique(data_final$id)) #should be 4052 ids

#Write the trimmed, combined csv file
write_csv(data_final, 'Combined_trial_data_with_120_4or8_trials_per_individual_dataset.csv')
...
data_full <- rbind(dataset1_expt1_subset, dataset1_expt2_subset, dataset2_subset)
data_full$block[is.na(data_full$block)] <- 1
#Create the unique id for each dataset in the combined file
data_full$id <- as.numeric(data_full$id)
data_full <- data_full %>% ungroup() %>%
  mutate(group = cumsum(id != lag(id, default = first(id))) + 1)

#Delete the original id column and replace it with the group column
data_full <- subset(data_full, select = -c(id))
data_full$id <- data_full$group
data_full <- subset(data_full, select = -c(group))

# Move the id col to the right place
data_full_final <- data_full %>% relocate(id, .after = experiment)

head(data_full_final)
dim(data_full_final) #Should be 827646 rows and 18 columns
length(unique(data_full_final$id)) #should be 4052 ids

#Write the trimmed, combined csv file
write_csv(data_full_final, 'Combined_trial_data_with_all_trials_in_all_datasets.csv')
